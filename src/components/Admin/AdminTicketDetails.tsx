...
export function AdminTicketDetails() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [ticketData, messageData] = await Promise.all([
        ticketService.getAdminById(id),
        ticketService.listAdminMessages(id),
      ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await ticketService.sendAdminMessage(ticket.id, { message: messageDraft.trim(), attachment: attachmentDraft });
      setMessageDraft('');
  ...
  const handleOpenAttachment = async (messageId: string, fileName?: string | null) => {
    ...
    try {
      const blob = await ticketService.viewAdminMessageAttachment(messageId);
        ...
        <Stack gap="lg">
          <Paper p="xl" radius="lg" style={{ background: `linear-gradient(135deg, ${DARK_BLUE} 0%, #16357f 100%)`, color: 'white' }}>
            <Group justify="space-between" align="flex-start">
              <Stack gap="sm">
                <Group gap="sm">
                  <ThemeIcon size={42} radius="md" variant="light" color="white"><LifeBuoy size={20} /></ThemeIcon>
                  <Title order={2} c="white">Chamado #{ticket ? shortTicketId(ticket.id) : shortTicketId(id)}</Title>
                </Group>
                <Text c="rgba(255,255,255,0.84)">Visão completa para triagem e acompanhamento assíncrono.</Text>
              </Stack>
              <Button variant="white" color="dark" leftSection={<ArrowLeft size={16} />} onClick={() => navigate('/adm-tickets')}>
                Voltar
              </Button>
            </Group>
          </Paper>
              ...
              <Paper withBorder radius="md" p="sm" style={{ background: adaptiveSubtleSurface }}>
                <Group justify="space-between" align="flex-start" gap="sm">
                  <Stack gap={4}>
                    ...
                    <Text size="xs" c="dimmed">Solicitante: {ticket.createdByName || 'Usuário não identificado'} {ticket.createdByEmail ? `(${ticket.createdByEmail})` : ''}</Text>
                    {ticket.branchName ? <Text size="xs" c="dimmed">Unidade: {ticket.branchName}</Text> : null}
                  </Stack>
                  <Stack gap={6} w={220}>
              ...
              <Paper withBorder radius="md" p="md">
                <Stack gap="md">
                  ...
                  <Textarea
                    ...
                    placeholder="Adicione um novo registro para o histórico do chamado..."
                    minRows={3}
                    autosize
                    value={messageDraft}
