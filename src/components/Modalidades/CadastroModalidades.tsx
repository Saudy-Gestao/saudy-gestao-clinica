  ...
  return (
    <Box bg="var(--mantine-color-body)" style={{ minHeight: '100vh' }}>
      ...
      <Box p={isMobile ? 'sm' : isTablet ? 'md' : 'xl'} maw={isMobile ? '100%' : 1400} mx="auto">
        ...
        <Paper className="cadastro-modalidades-panel" p={isMobile ? 'sm' : 'lg'} withBorder radius="md" mb={isMobile ? 'md' : 'lg'}>
        ...
        {itemsLoading ? (
          isMobile ? (
            ...
            <Box className="cadastro-modalidades-table-wrap">
          ...
          isMobile ? (
            ...
            >
              <Table horizontalSpacing="md" verticalSpacing="md">
                <Table.Thead>
                  <Table.Tr>
                    <Table.Th className="cadastro-modalidades-th">Nome</Table.Th>
                    <Table.Th className="cadastro-modalidades-th">Status</Table.Th>
                    {!isTablet && <Table.Th className="cadastro-modalidades-th">Última alteração</Table.Th>}
                    <Table.Th className="cadastro-modalidades-th" style={{ textAlign: 'center', width: 96 }}>
                      Ações
                    </Table.Th>
                  </Table.Tr>
                </Table.Thead>
                <Table.Tbody>
                  {filtered.length === 0 ? (
                    ...
                    paginatedItems.map((it) => (
                      <Table.Tr key={it.id} className="cadastro-modalidades-row">
                        <Table.Td>
                          <Stack gap={2}>
                            <Text fw={600} size="sm">{it.name}</Text>
      ...
      >
        <Stack gap={10}>
          ...
          {editingId ? (
            <Box p="md" className="ui-toggle-card">
              <Group justify="space-between" align="center" wrap="wrap" gap="sm">
                <Box>
                  <Text fw={600} size="sm">Modalidade ativa</Text>
                  <Text size="xs" c="dimmed">Modalidades inativas deixam de ficar disponíveis para novos procedimentos.</Text>
                </Box>
                <Switch
          ...
          {similarWarning && similarWarning.length > 0 ? (
            <Paper withBorder p="sm" radius="md" className="cadastro-modalidades-warning">
          ...
          <Group justify="flex-end" mt={8}>
            ...
            <Button onClick={() => submit(false)} size="sm" loading={saving} disabled={saving}>
