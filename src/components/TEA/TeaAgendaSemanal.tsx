import { useEffect, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Badge, Box, Button, DateInput, Divider, Group, MultiSelect, Paper, Skeleton, Stack, Text, TextInput, Tooltip } from '@/components/ui';
import { useElementSize, useMediaQuery } from '@/components/ui';
import { showNotification } from '@/components/ui';
import { ChevronLeft, ChevronRight, Clock3, Search } from 'lucide-react';
import dayjs from 'dayjs';
import 'dayjs/locale/pt-br';
import { Header } from '../Header/Header';
import { useTeaWeeklyAgendaQuery, type TeaAgendaItem } from '../../hooks/useTeaWeeklyAgendaQuery';
import { resolveApiErrorMessage } from '../../lib/apiError';
import { AGENDA_CARD_DENSITY, formatCardInitials, resolveAgendaCardPresentation, type AgendaCardDetailLevel } from '../../utils/agendaCardDensity';
import './TeaAgendaSemanal.css';

const WEEKDAY_LABELS = ['Seg', 'Ter', 'Qua', 'Qui', 'Sex'];
const SLOTS = ['08:00', '08:45', '09:30', '10:15', '11:00', '11:45', '12:30', '13:15', '14:00', '14:45', '15:30', '16:15', '17:00', '17:45', '18:30', '19:15'];

const LEGEND_ITEMS: Array<[string, string]> = [
  ['teal', 'Agendado'],
  ['violet', 'Reservado'],
  ['yellow', 'Pendente'],
  ['orange', 'Bloqueado'],
];

const MOCK_PATIENTS = [
  ['Miguel Torres', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '08:00', 'CONFIRMED'], ['Helena Pires', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Central)', '09:30', 'SCHEDULED'], ['Davi Rocha', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Central)', '11:00', 'CONFIRMED'], ['Alice Barros', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Central)', '14:00', 'RESERVED'], ['Bernardo Melo', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '15:30', 'SCHEDULED'], ['Sofia Ramos', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Sul)', '08:45', 'CONFIRMED'], ['Enzo Martins', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Sul)', '10:15', 'CONFIRMED'], ['Laura Nunes', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Sul)', '16:15', 'PENDING'], ['Isabela Costa', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '09:30', 'CONFIRMED'], ['Gabriel Lima', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Central)', '13:15', 'SCHEDULED'], ['João Vitor', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Central)', '15:30', 'RESERVED'], ['Ana Beatriz', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Central)', '17:00', 'CONFIRMED'], ['Pedro Souza', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Sul)', '11:00', 'SCHEDULED'], ['Maria Fernandes', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Sul)', '14:45', 'CONFIRMED'], ['Lucas Andrade', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Sul)', '18:30', 'PENDING'],
  ['Valentina Alves', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '10:15', 'RESERVED'], ['Arthur Mendes', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Central)', '11:45', 'PENDING'], ['Manuela Castro', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Central)', '08:45', 'RESERVED'], ['Nicolas Freitas', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Sul)', '13:15', 'PENDING'], ['Lívia Martins', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Sul)', '17:00', 'RESERVED'], ['Ravi Oliveira', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Sul)', '15:30', 'PENDING'], ['Clara Monteiro', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Sul)', '09:30', 'RESERVED'], ['Bento Cardoso', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '18:30', 'PENDING'], ['Yasmin Ribeiro', 'Terapia ocupacional', 'Beatriz Alves', 'Sala 3 (Unidade Central)', '14:45', 'RESERVED'], ['Cecília Ramos', 'Psicologia', 'Camila Duarte', 'Sala 1 (Unidade Central)', '12:30', 'CANCELED'], ['Gustavo Martins', 'Fonoaudiologia', 'Rafael Nogueira', 'Sala 2 (Unidade Sul)', '17:45', 'CANCELADO'], ['Marina Lopes', 'Fisioterapia', 'Thiago Prado', 'Sala 4 (Unidade Central)', '19:15', 'CANCELED'], ['', '', '', 'Sala 4 (Unidade Central)', '12:30', 'BLOCKED'], ['', '', '', 'Sala 2 (Unidade Sul)', '12:30', 'BLOQUEADO'], ['', '', '', 'Sala 3 (Unidade Central)', '08:00', 'BLOCKED'],
] as const;

const createMockAppointments = (weekStart: dayjs.Dayjs): TeaAgendaItem[] => {
  const occurrencesByRoom = new Map<string, number>();
  const appointments = Array.from({ length: 3 }, (_, round) => MOCK_PATIENTS.map((item, index) => {
    const occurrence = occurrencesByRoom.get(item[3]) || 0;
    occurrencesByRoom.set(item[3], occurrence + 1);
    return { id: `tea-demo-${round}-${index}`, patientName: item[0], specialty: item[1], doctorName: item[2], roomName: item[3], time: SLOTS[(Math.floor(occurrence / 5) * 4 + index % 4) % SLOTS.length], status: item[5], date: weekStart.add(occurrence % 5, 'day').format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' as const };
  })).flat();
  const simultaneousAppointments: TeaAgendaItem[] = [
    { id: 'tea-demo-simultaneous-1', patientName: 'Mariana Azevedo', specialty: 'Psicologia', doctorName: 'Camila Duarte', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'SCHEDULED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-2', patientName: 'Thiago Mendonça', specialty: 'Terapia ocupacional', doctorName: 'Beatriz Alves', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'RESERVED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-4', patientName: 'Júlia Martins', specialty: 'Fonoaudiologia', doctorName: 'Rafael Nogueira', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'PENDING', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-5', patientName: 'Pedro Henrique', specialty: 'Fisioterapia', doctorName: 'Thiago Prado', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'CANCELED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-6', patientName: 'Luiza Fernandes', specialty: 'Psicologia', doctorName: 'Camila Duarte', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'SCHEDULED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-7', patientName: 'Caio Almeida', specialty: 'Terapia ocupacional', doctorName: 'Beatriz Alves', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'RESERVED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-8', patientName: 'Bianca Costa', specialty: 'Fisioterapia', doctorName: 'Thiago Prado', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'SCHEDULED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-9', patientName: 'Rafael Santos', specialty: 'Psicologia', doctorName: 'Camila Duarte', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'PENDING', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-10', patientName: 'Aline Souza', specialty: 'Fonoaudiologia', doctorName: 'Rafael Nogueira', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'RESERVED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-11', patientName: 'Diego Lima', specialty: 'Terapia ocupacional', doctorName: 'Beatriz Alves', roomName: 'Sala 1 (Unidade Central)', time: '09:30', status: 'CANCELED', date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
    { id: 'tea-demo-simultaneous-3', patientName: 'Larissa Freitas', specialty: 'Fonoaudiologia', doctorName: 'Rafael Nogueira', roomName: 'Sala 1 (Unidade Central)', time: '14:00', status: 'PENDING', date: weekStart.add(2, 'day').format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' },
  ];
  const mixedRoomSlots: TeaAgendaItem[] = [
    { time: '08:00', patients: [['Marina Alves', 'Psicologia', 'Camila Duarte', 'SCHEDULED'], ['Otávio Ribeiro', 'Fisioterapia', 'Thiago Prado', 'RESERVED']] },
    { time: '10:15', patients: [['Beatriz Moura', 'Fonoaudiologia', 'Rafael Nogueira', 'SCHEDULED'], ['Henrique Dias', 'Terapia ocupacional', 'Beatriz Alves', 'PENDING'], ['Lorena Campos', 'Psicologia', 'Camila Duarte', 'RESERVED']] },
    { time: '11:45', patients: [['Alice Martins', 'Fisioterapia', 'Thiago Prado', 'SCHEDULED'], ['Guilherme Reis', 'Psicologia', 'Camila Duarte', 'PENDING'], ['Nathalia Souza', 'Fonoaudiologia', 'Rafael Nogueira', 'RESERVED'], ['Samuel Costa', 'Terapia ocupacional', 'Beatriz Alves', 'CANCELED'], ['Isadora Lima', 'Psicologia', 'Camila Duarte', 'SCHEDULED']] },
    { time: '13:15', patients: [['Rafaela Nunes', 'Fonoaudiologia', 'Rafael Nogueira', 'SCHEDULED'], ['Matheus Freire', 'Fisioterapia', 'Thiago Prado', 'RESERVED'], ['Elisa Rocha', 'Psicologia', 'Camila Duarte', 'PENDING'], ['Bruno Martins', 'Terapia ocupacional', 'Beatriz Alves', 'SCHEDULED'], ['Sabrina Lopes', 'Fonoaudiologia', 'Rafael Nogueira', 'RESERVED'], ['Vitor Hugo', 'Psicologia', 'Camila Duarte', 'CANCELED']] },
    { time: '15:30', patients: [['Amanda Castro', 'Psicologia', 'Camila Duarte', 'SCHEDULED'], ['João Pedro', 'Fisioterapia', 'Thiago Prado', 'SCHEDULED'], ['Carolina Mendes', 'Fonoaudiologia', 'Rafael Nogueira', 'RESERVED'], ['Felipe Araujo', 'Terapia ocupacional', 'Beatriz Alves', 'PENDING'], ['Manuela Alves', 'Psicologia', 'Camila Duarte', 'SCHEDULED'], ['Diego Martins', 'Fonoaudiologia', 'Rafael Nogueira', 'RESERVED'], ['Yasmin Oliveira', 'Fisioterapia', 'Thiago Prado', 'CANCELED']] },
  ].flatMap(({ time, patients }, groupIndex) => patients.map(([patientName, specialty, doctorName, status], patientIndex) => ({ id: `tea-demo-mixed-${groupIndex}-${patientIndex}`, patientName, specialty, doctorName, roomName: 'Sala 1 (Unidade Central)', time, status, date: weekStart.format('YYYY-MM-DD'), type: 'ATENDIMENTO TEA', source: 'APPOINTMENT' as const })));
  return [...appointments, ...simultaneousAppointments, ...mixedRoomSlots];
};

const toWeekStartMonday = (value: dayjs.Dayjs) => value.subtract((value.day() + 6) % 7, 'day').startOf('day');
const isoDateToLocalDate = (value: string) => {
  const [year, month, day] = value.split('-').map(Number);
  return year && month && day ? new Date(year, month - 1, day, 12) : null;
};
const roomUnit = (room: string) => room.match(/\(([^)]+)\)/)?.[1] || 'Unidade não informada';
const roomDisplay = (room: string) => ({ name: room.replace(/\s*\([^)]*\)\s*$/, '').trim() || room, unit: roomUnit(room) });
const isCanceledStatus = (value?: string) => ['CANCELED', 'CANCELADO'].includes(String(value || '').toUpperCase());
const statusLabel = (value?: string) => {
  const v = String(value || '').toUpperCase();
  if (['CANCELED', 'CANCELADO'].includes(v)) return 'Cancelado';
  if (['BLOCKED', 'BLOQUEADO'].includes(v)) return 'Bloqueado';
  if (['RESERVED', 'RESERVADO'].includes(v)) return 'Reservado';
  if (['PENDING', 'PENDENTE'].includes(v)) return 'Pendente';
  return 'Agendado';
};
const statusColor = (value?: string) => {
  const v = String(value || '').toUpperCase();
  if (['CANCELED', 'CANCELADO'].includes(v)) return 'red';
  if (['BLOCKED', 'BLOQUEADO'].includes(v)) return 'orange';
  if (['RESERVED', 'RESERVADO'].includes(v)) return 'violet';
  if (['PENDING', 'PENDENTE'].includes(v)) return 'yellow';
  return 'teal';
};

function AppointmentCard({ item, compact = false, detailLevel = 'minimal', patientOnly = false, showInitials = false, hideContent = false }: { item: TeaAgendaItem; compact?: boolean; detailLevel?: AgendaCardDetailLevel; patientOnly?: boolean; showInitials?: boolean; hideContent?: boolean }) {
  const tooltipContent = (compact && detailLevel === 'minimal') || patientOnly ? (
    <Box className="tea-agenda-tooltip">
      <Group justify="space-between" gap="sm" mb={6} wrap="nowrap">
        <Text size="sm" fw={700}>{item.patientName || 'Detalhes do atendimento'}</Text>
        <Badge size="sm" variant="light" color={statusColor(item.status)}>{item.time}</Badge>
      </Group>
      <Text size="xs" c="dimmed" mb={4}>{item.roomName || 'Sala não vinculada'}</Text>
      <Stack gap={3}>
        <Text size="sm" fw={600}>{item.specialty || 'Terapia não informada'}</Text>
        <Text size="xs">Terapeuta: {item.doctorName || 'Não informado'}</Text>
        <Text size="xs" c="dimmed">Status: {statusLabel(item.status)}</Text>
      </Stack>
    </Box>
  ) : compact && detailLevel === 'summary' ? (
    <Box className="tea-agenda-tooltip">
      <Group justify="space-between" gap="sm" mb={6} wrap="nowrap">
        <Text size="sm" fw={700}>Detalhes do atendimento</Text>
        <Badge size="sm" variant="light" color={statusColor(item.status)}>{item.time}</Badge>
      </Group>
      <Text size="xs" c="dimmed" mb={4}>{item.roomName || 'Sala não vinculada'}</Text>
      <Text size="xs">Terapeuta: {item.doctorName || 'Não informado'}</Text>
      <Text size="xs" c="dimmed">Status: {statusLabel(item.status)}</Text>
    </Box>
  ) : (
    <Box className="tea-agenda-tooltip">
      <Group justify="space-between" gap="sm" mb={6} wrap="nowrap">
        <Text size="sm" fw={700}>Detalhes do atendimento</Text>
        <Badge size="sm" variant="light" color={statusColor(item.status)}>{item.time}</Badge>
      </Group>
      <Text size="xs" c="dimmed">{item.roomName || 'Sala não vinculada'}</Text>
      <Text size="xs" c="dimmed">Status: {statusLabel(item.status)}</Text>
    </Box>
  );

  return (
    <Tooltip label={tooltipContent} withArrow position="top" offset={8} openDelay={180} multiline>
      {compact ? (
        detailLevel !== 'minimal' ? (
          <Box
            aria-label={`${item.patientName || statusLabel(item.status)} · ${item.time}`}
            className="tea-agenda-chip"
            data-status={statusColor(item.status)}
            data-layout={showInitials ? 'row' : 'column'}
            data-centered={showInitials || hideContent ? 'true' : undefined}
            data-empty={hideContent ? 'true' : undefined}
          >
            {!hideContent && (
              <>
                <Text fw={700} size="xs" ta={showInitials ? 'center' : undefined} truncate>
                  {showInitials ? formatCardInitials(item.patientName) : (item.patientName || statusLabel(item.status))}
                </Text>
                {!patientOnly && <Text size="xs" c="dimmed" truncate>{item.specialty || 'Terapia não informada'}</Text>}
                {!patientOnly && detailLevel === 'full' && <Text size="xs" c="dimmed" truncate>{item.doctorName || 'Terapeuta não informado'}</Text>}
              </>
            )}
          </Box>
        ) : (
          <Box aria-label={`${item.patientName || statusLabel(item.status)} · ${item.time}`} className="tea-agenda-chip tea-agenda-chip--dot" data-status={statusColor(item.status)} />
        )
      ) : (
        <Paper radius="sm" withBorder className="tea-agenda-card" data-status={statusColor(item.status)}>
          <Text fw={700} size="xs" truncate>{item.patientName || statusLabel(item.status)}</Text>
          {item.patientName ? (
            <>
              <Text size="xs" c="dimmed" className="tea-agenda-card__meta" truncate>{item.specialty || 'Terapia não informada'}</Text>
              <Text size="xs" c="dimmed" className="tea-agenda-card__meta" truncate>{item.doctorName || 'Terapeuta não informado'}</Text>
            </>
          ) : (
            <Text size="xs" c="dimmed" className="tea-agenda-card__meta" truncate>Indisponível para atendimento</Text>
          )}
        </Paper>
      )}
    </Tooltip>
  );
}

export function TeaAgendaSemanal() {
  const navigate = useNavigate();
  const isMobile = useMediaQuery('(max-width: 799px)');
  const { ref: scheduleRef, width: scheduleWidth } = useElementSize();
  const { data: apiAppointments = [], isLoading: loading, error } = useTeaWeeklyAgendaQuery();
  const [mode, setMode] = useState<'day' | 'week'>('week');
  const [selectedDate, setSelectedDate] = useState(() => dayjs().format('YYYY-MM-DD'));
  const [weekStart, setWeekStart] = useState(() => toWeekStartMonday(dayjs()));
  const [search, setSearch] = useState('');
  const [unitFilter, setUnitFilter] = useState<string[]>([]);
  const [specialtyFilter, setSpecialtyFilter] = useState<string[]>([]);
  const [doctorFilter, setDoctorFilter] = useState<string[]>([]);
  const [roomFilter, setRoomFilter] = useState<string[]>([]);
  const [patientFilter, setPatientFilter] = useState<string[]>([]);

  const weekStartIso = weekStart.format('YYYY-MM-DD');
  const weekEndIso = weekStart.add(4, 'day').format('YYYY-MM-DD');
  const hasRealDataForWeek = apiAppointments.some((item) => item.date >= weekStartIso && item.date <= weekEndIso);
  const isDemo = import.meta.env.DEV && !loading && !hasRealDataForWeek;
  const allAppointments = useMemo(() => (isDemo ? createMockAppointments(weekStart) : apiAppointments), [apiAppointments, isDemo, weekStart]);
  const visibleAppointments = useMemo(() => allAppointments.filter((item) => !isCanceledStatus(item.status)), [allAppointments]);
  const days = useMemo(() => Array.from({ length: 5 }, (_, index) => weekStart.add(index, 'day')), [weekStart]);

  const options = useMemo(() => ({
    units: Array.from(new Set(visibleAppointments.map((item) => roomUnit(item.roomName)).filter(Boolean))).sort(),
    specialties: Array.from(new Set(visibleAppointments.map((item) => item.specialty).filter(Boolean))).sort(),
    doctors: Array.from(new Set(visibleAppointments.map((item) => item.doctorName).filter(Boolean))).sort(),
    rooms: Array.from(new Set(visibleAppointments.map((item) => item.roomName).filter(Boolean))).sort(),
    patients: Array.from(new Set(visibleAppointments.map((item) => item.patientName).filter(Boolean))).sort(),
  }), [visibleAppointments]);

  const filtered = useMemo(() => visibleAppointments.filter((item) => {
    const query = search.trim().toLowerCase();
    const matchesQuery = !query || [item.patientName, item.specialty, item.doctorName, item.roomName, item.time].some((value) => String(value || '').toLowerCase().includes(query));
    return matchesQuery
      && (!unitFilter.length || unitFilter.includes(roomUnit(item.roomName)))
      && (!specialtyFilter.length || specialtyFilter.includes(item.specialty))
      && (!doctorFilter.length || doctorFilter.includes(item.doctorName))
      && (!roomFilter.length || roomFilter.includes(item.roomName))
      && (!patientFilter.length || patientFilter.includes(item.patientName));
  }), [visibleAppointments, search, unitFilter, specialtyFilter, doctorFilter, roomFilter, patientFilter]);

  const bySlot = useMemo(() => {
    const map = new Map<string, TeaAgendaItem[]>();
    filtered.forEach((item) => {
      const key = `${item.date}#${item.time}`;
      map.set(key, [...(map.get(key) || []), item]);
    });
    return map;
  }, [filtered]);

  const rooms = useMemo(() => Array.from(new Set(filtered.map((item) => item.roomName).filter(Boolean))).sort(), [filtered]);

  // A room can have more than one agenda running at the same time slot. Rather than
  // squeezing every concurrent appointment into one crammed cell, each room gets one
  // repeated column ("lane") per concurrent agenda it actually needs — so every
  // appointment gets its own full-width slot, side by side, like the room map pattern.
  // Lane count is scoped to the dates currently on screen so a busy week elsewhere
  // doesn't force empty lanes onto a quiet one.
  const visibleDateSet = useMemo(() => (
    mode === 'day' ? new Set([selectedDate]) : new Set(days.map((day) => day.format('YYYY-MM-DD')))
  ), [mode, selectedDate, days]);

  const roomLanes = useMemo(() => {
    const concurrencyByRoom = new Map<string, number>();
    const bucketCounts = new Map<string, number>();
    filtered.forEach((item) => {
      if (!item.roomName || !visibleDateSet.has(item.date)) return;
      const key = `${item.date}#${item.time}#${item.roomName}`;
      bucketCounts.set(key, (bucketCounts.get(key) || 0) + 1);
    });
    bucketCounts.forEach((count, key) => {
      const room = key.split('#')[2];
      concurrencyByRoom.set(room, Math.max(concurrencyByRoom.get(room) || 1, count));
    });
    return rooms.flatMap((room) => (
      Array.from({ length: concurrencyByRoom.get(room) || 1 }, (_, laneIndex) => ({ room, laneIndex }))
    ));
  }, [filtered, rooms, visibleDateSet]);

  const getLaneItem = (date: string, time: string, room: string, laneIndex: number) => {
    const items = (bySlot.get(`${date}#${time}`) || []).filter((item) => item.roomName === room);
    if (items.length <= 1) return laneIndex === 0 ? items[0] : undefined;
    return [...items].sort((a, b) => a.id.localeCompare(b.id))[laneIndex];
  };

  const getScheduleCellWidth = () => {
    const laneCount = Math.max(roomLanes.length, 1);
    const columnCount = mode === 'day' ? laneCount : days.length * laneCount;
    const timeColumnWidth = mode === 'day' ? 110 : 90;
    const fallbackWidth = mode === 'day' ? Math.max(760, 132 + laneCount * 142) : Math.max(1120, 90 + days.length * laneCount * 112);
    const gridWidth = Math.max(scheduleWidth, fallbackWidth);
    return Math.max(40, (gridWidth - timeColumnWidth - columnCount * 6) / columnCount);
  };

  const renderLaneCell = (item: TeaAgendaItem | undefined, compact = false, detailLevel: AgendaCardDetailLevel = 'minimal') => {
    if (!item) return compact ? null : <Text size="xs" c="dimmed" ta="center" py={14}>Disponível</Text>;
    const cellWidth = getScheduleCellWidth();
    if (!compact) {
      if (!item.patientName) return <AppointmentCard item={item} />;
      const presentation = resolveAgendaCardPresentation(cellWidth, 1, 'full');
      if (presentation.detailLevel === 'full' && !presentation.patientOnly) return <AppointmentCard item={item} />;
      return <Box className="tea-agenda-cell-fill"><AppointmentCard item={item} compact {...presentation} /></Box>;
    }
    // Compact-density rows (see `rowDensity`) render a fixed, short row height —
    // matching `.tea-agenda-row[data-density='compact'] .tea-agenda-cell` in
    // TeaAgendaSemanal.css — so a chip there only has room for one line
    // regardless of how wide its column is; anything that doesn't fit stays
    // available in the hover tooltip.
    const cellHeight = rowDensity === 'compact' ? 36 : 56;
    const presentation = resolveAgendaCardPresentation(cellWidth, 1, detailLevel, cellHeight);
    return <Box className="tea-agenda-cell-fill"><AppointmentCard item={item} compact {...presentation} /></Box>;
  };

  const weeklyDetailLevel: AgendaCardDetailLevel = 'full';
  const showWeeklyCardDetails = getScheduleCellWidth() >= AGENDA_CARD_DENSITY.summaryMinWidth;
  const weekTotal = filtered.filter((item) => item.date >= weekStart.format('YYYY-MM-DD') && item.date <= weekStart.add(4, 'day').format('YYYY-MM-DD')).length;
  const activeAppointmentsCount = filtered.filter((item) => !isCanceledStatus(item.status)).length;
  const dateLabel = dayjs(selectedDate).locale('pt-br').format('dddd, DD [de] MMMM [de] YYYY');

  useEffect(() => { dayjs.locale('pt-br'); }, []);
  useEffect(() => {
    if (error) showNotification({ title: 'Erro', message: resolveApiErrorMessage(error as any, 'Erro ao carregar agenda semanal de Terapias'), color: 'red' });
  }, [error]);

  const moveDay = (amount: number) => {
    const next = dayjs(selectedDate).add(amount, 'day');
    setSelectedDate(next.format('YYYY-MM-DD'));
    setWeekStart(toWeekStartMonday(next));
  };
  const setToday = () => {
    const today = dayjs();
    setSelectedDate(today.format('YYYY-MM-DD'));
    setWeekStart(toWeekStartMonday(today));
  };

  const rowDensity = showWeeklyCardDetails ? 'expanded' : 'compact';

  return (
    <Box className="tea-agenda-page">
      <Header back={{ label: 'Voltar', onClick: () => navigate('/tea') }} />
      <Box p={isMobile ? 'sm' : 'xl'} w="100%" className="tea-agenda-shell">
        <Box className="tea-agenda-hero">
          <Text className="tea-agenda-eyebrow">OPERAÇÃO CLÍNICA · TERAPIAS</Text>
          <Text className="tea-agenda-title" fw={700} size="2xl">Agenda semanal de Terapias</Text>
          <Text className="tea-agenda-subtitle" size="sm">Acompanhe os atendimentos por paciente, terapeuta e sala</Text>
        </Box>

        <Paper p={isMobile ? 'sm' : 'md'} withBorder className="tea-agenda-panel">
          <Stack gap="md">
            <Group justify="flex-end" align="center">
              <Group gap={6} className="tea-agenda-mode-toggle">
                <Button size="xs" variant={mode === 'day' ? 'filled' : 'default'} onClick={() => setMode('day')}>Dia</Button>
                <Button size="xs" variant={mode === 'week' ? 'filled' : 'default'} onClick={() => setMode('week')}>Semana</Button>
              </Group>
            </Group>

            <Box className="tea-agenda-filters">
              <MultiSelect label="Unidade" placeholder="Todas as unidades" data={options.units} value={unitFilter} onChange={setUnitFilter} searchable clearable />
              <MultiSelect label="Especialidade" placeholder="Todas as especialidades" data={options.specialties} value={specialtyFilter} onChange={setSpecialtyFilter} searchable clearable />
              <MultiSelect label="Terapeuta" placeholder="Todos os terapeutas" data={options.doctors} value={doctorFilter} onChange={setDoctorFilter} searchable clearable />
              <MultiSelect label="Sala" placeholder="Todas as salas" data={options.rooms} value={roomFilter} onChange={setRoomFilter} searchable clearable />
              <MultiSelect label="Paciente" placeholder="Todos os pacientes" data={options.patients} value={patientFilter} onChange={setPatientFilter} searchable clearable />
            </Box>

            <Group justify="space-between" align="center" wrap="wrap" gap="sm">
              <Box className="tea-agenda-toolbar">
                <Group gap="xs" wrap="wrap">
                  {mode === 'day' ? (
                    <>
                      <Button size="xs" variant="default" onClick={() => moveDay(-1)} leftSection={<ChevronLeft size={14} />}>Dia anterior</Button>
                      <Button size="xs" variant="light" onClick={setToday}>Hoje</Button>
                      <Button size="xs" variant="default" onClick={() => moveDay(1)} rightSection={<ChevronRight size={14} />}>Próximo dia</Button>
                      <DateInput
                        className="tea-agenda-day-picker"
                        label="Selecionar data"
                        value={isoDateToLocalDate(selectedDate)}
                        onChange={(date) => {
                          if (!date) return;
                          const next = dayjs(date);
                          setSelectedDate(next.format('YYYY-MM-DD'));
                          setWeekStart(toWeekStartMonday(next));
                        }}
                      />
                      <Divider orientation="vertical" my={4} />
                      <Group gap={2} wrap="nowrap">
                        {WEEKDAY_LABELS.map((label, index) => {
                          const day = weekStart.add(index, 'day');
                          const selected = selectedDate === day.format('YYYY-MM-DD');
                          return (
                            <Button key={label} size="xs" variant={selected ? 'filled' : 'subtle'} px={10} onClick={() => setSelectedDate(day.format('YYYY-MM-DD'))} aria-label={`Selecionar ${label}`}>
                              {label}
                            </Button>
                          );
                        })}
                      </Group>
                    </>
                  ) : (
                    <>
                      <Button size="xs" variant="default" onClick={() => setWeekStart((prev) => prev.subtract(7, 'day'))} leftSection={<ChevronLeft size={14} />}>Semana anterior</Button>
                      <Button size="xs" variant="light" onClick={setToday}>Hoje</Button>
                      <Button size="xs" variant="default" onClick={() => setWeekStart((prev) => prev.add(7, 'day'))} rightSection={<ChevronRight size={14} />}>Próxima semana</Button>
                    </>
                  )}
                </Group>
              </Box>
              <Group gap="xs" align="center" wrap="wrap">
                <TextInput
                  placeholder="Buscar por paciente, sala, terapeuta ou horário"
                  value={search}
                  onChange={(event) => setSearch(event.currentTarget.value)}
                  rightSection={<Search size={17} />}
                  className="tea-agenda-search"
                />
                <Text size="sm" c="dimmed">
                  {mode === 'day' ? dateLabel : `${weekStart.format('DD/MM/YYYY')} até ${weekStart.add(4, 'day').format('DD/MM/YYYY')}`}
                </Text>
              </Group>
            </Group>

            <Box className="tea-agenda-stats">
              <Paper withBorder radius="md" p="sm" className="tea-agenda-stat-card">
                <Text size="xl" fw={700}>{weekTotal}</Text>
                <Text size="sm" fw={600}>Atendimentos</Text>
                <Text size="xs" c="dimmed">na semana exibida</Text>
              </Paper>
              <Paper withBorder radius="md" p="sm" className="tea-agenda-stat-card">
                <Text size="xl" fw={700}>{new Set(filtered.map((item) => item.patientName)).size}</Text>
                <Text size="sm" fw={600}>Pacientes</Text>
                <Text size="xs" c="dimmed">com atendimento</Text>
              </Paper>
              <Paper withBorder radius="md" p="sm" className="tea-agenda-stat-card">
                <Text size="xl" fw={700}>{rooms.length}</Text>
                <Text size="sm" fw={600}>Salas</Text>
                <Text size="xs" c="dimmed">em uso na visão</Text>
              </Paper>
              <Paper withBorder radius="md" p="sm" className="tea-agenda-stat-card tea-agenda-stat-card--highlight">
                <Text size="xl" fw={700}>{activeAppointmentsCount}</Text>
                <Text size="sm" fw={600}>Agendados</Text>
                <Text size="xs" c="dimmed">atendimentos ativos</Text>
              </Paper>
            </Box>

            <Group gap="lg" className="tea-agenda-legend">
              {LEGEND_ITEMS.map(([color, label]) => (
                <Group key={color} gap={6}>
                  <Box className="tea-agenda-legend-dot" data-color={color} />
                  <Text size="xs" c="dimmed">{label}</Text>
                </Group>
              ))}
            </Group>

            {loading ? (
              <Box className="tea-agenda-skeleton-row">
                {Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} height={500} radius="md" />)}
              </Box>
            ) : (
              <Box ref={scheduleRef} className="tea-agenda-schedule">
                {mode === 'day' ? (
                  <Box style={{ minWidth: Math.max(760, 132 + roomLanes.length * 142) }}>
                    <Box className="tea-agenda-head-row" style={{ gridTemplateColumns: `110px repeat(${Math.max(roomLanes.length, 1)}, minmax(128px, 1fr))` }}>
                      <Box className="tea-agenda-head-cell tea-agenda-head-cell--time">
                        <Text size="xs" fw={700} c="dimmed">HORÁRIO</Text>
                        <Text size="xs" c="dimmed">{dayjs(selectedDate).format('DD/MM')}</Text>
                      </Box>
                      {roomLanes.map((lane) => (
                        <Box key={`${lane.room}__${lane.laneIndex}`} className="tea-agenda-head-cell tea-agenda-head-cell--active">
                          <Text fw={700} size="sm" truncate>{roomDisplay(lane.room).name}</Text>
                          <Text size="xs" c="dimmed">{roomDisplay(lane.room).unit}</Text>
                        </Box>
                      ))}
                    </Box>
                    {SLOTS.map((slot) => (
                      <Box key={slot} className="tea-agenda-row" data-density={rowDensity} style={{ gridTemplateColumns: `110px repeat(${Math.max(roomLanes.length, 1)}, minmax(128px, 1fr))` }}>
                        <Box className="tea-agenda-row-time">
                          <Group gap={5} wrap="nowrap"><Clock3 size={13} /><Text size="xs" c="dimmed">{slot}</Text></Group>
                        </Box>
                        {roomLanes.length ? roomLanes.map((lane) => (
                          <Box key={`${slot}-${lane.room}-${lane.laneIndex}`} className="tea-agenda-cell">
                            {renderLaneCell(getLaneItem(selectedDate, slot, lane.room, lane.laneIndex))}
                          </Box>
                        )) : (
                          <Box className="tea-agenda-cell"><Text size="xs" c="dimmed">Nenhum resultado</Text></Box>
                        )}
                      </Box>
                    ))}
                  </Box>
                ) : (
                  <Box style={{ minWidth: Math.max(1120, 90 + days.length * Math.max(roomLanes.length, 1) * 112) }}>
                    <Box className="tea-agenda-head-row" style={{ gridTemplateColumns: `90px repeat(${days.length * Math.max(roomLanes.length, 1)}, minmax(100px, 1fr))` }}>
                      <Box className="tea-agenda-head-cell tea-agenda-head-cell--time">
                        <Text size="xs" fw={700} c="dimmed">HORÁRIO</Text>
                      </Box>
                      {days.map((day, index) => (
                        <Box key={day.format('YYYY-MM-DD')} className="tea-agenda-head-cell tea-agenda-head-cell--active" style={{ gridColumn: `span ${Math.max(roomLanes.length, 1)}` }}>
                          <Text fw={700} size="sm">{WEEKDAY_LABELS[index]} · {day.format('DD/MM')}</Text>
                          <Text size="xs" c="dimmed">{filtered.filter((item) => item.date === day.format('YYYY-MM-DD')).length} atendimento(s)</Text>
                        </Box>
                      ))}
                    </Box>
                    <Box className="tea-agenda-subhead-row" style={{ gridTemplateColumns: `90px repeat(${days.length * Math.max(roomLanes.length, 1)}, minmax(100px, 1fr))` }}>
                      <Box />
                      {days.flatMap((day) => (
                        roomLanes.length ? roomLanes.map((lane) => (
                          <Tooltip key={`${day.format('YYYY-MM-DD')}-${lane.room}-${lane.laneIndex}`} label={lane.room} withArrow openDelay={180} multiline>
                            <Box className="tea-agenda-subhead-cell">
                              <Text size="xs" c="dimmed" ta="center">{roomDisplay(lane.room).name}</Text>
                              <Text size="xs" c="dimmed" ta="center">{roomDisplay(lane.room).unit}</Text>
                            </Box>
                          </Tooltip>
                        )) : [<Box key={`${day.format('YYYY-MM-DD')}-empty`} />]
                      ))}
                    </Box>
                    {SLOTS.map((slot) => (
                      <Box key={slot} className="tea-agenda-row" data-density={rowDensity} style={{ gridTemplateColumns: `90px repeat(${days.length * Math.max(roomLanes.length, 1)}, minmax(100px, 1fr))` }}>
                        <Box className="tea-agenda-row-time">
                          <Text size="xs" c="dimmed">{slot}</Text>
                        </Box>
                        {days.flatMap((day) => (
                          roomLanes.length ? roomLanes.map((lane) => (
                            <Box key={`${day.format('YYYY-MM-DD')}-${slot}-${lane.room}-${lane.laneIndex}`} className="tea-agenda-cell">
                              {renderLaneCell(getLaneItem(day.format('YYYY-MM-DD'), slot, lane.room, lane.laneIndex), true, weeklyDetailLevel)}
                            </Box>
                          )) : [<Box key={`${day.format('YYYY-MM-DD')}-${slot}-empty`} className="tea-agenda-cell"><Text size="xs" c="dimmed">Nenhum resultado</Text></Box>]
                        ))}
                      </Box>
                    ))}
                  </Box>
                )}
              </Box>
            )}

            {!loading && !filtered.length && <Text ta="center" c="dimmed" py="xl">Nenhum atendimento encontrado com os filtros atuais.</Text>}
            {!loading && <Text size="xs" c="dimmed">Passe o mouse sobre um atendimento para consultar os detalhes.</Text>}
          </Stack>
        </Paper>
      </Box>
    </Box>
  );
}
